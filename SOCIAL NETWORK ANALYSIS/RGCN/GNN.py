...
from models import RGCN, GAT, GCN, SAGE, SGC, ComplexRGCN
...
parser.add_argument("-dataset", type=str, default='Twibot20', help="dataset", choices=['Twibot20'])
parser.add_argument("-ensemble", type=bool, default=True, help="whether use ensemble")
parser.add_argument('-model', type=str, default='GCN', choices=['GCN', 'GAT', 'GraphSage', 'RGCN', 'SGC', 'ComplexRGCN'])
...
def main(seed):
    ...
    for num in range(model_num):
        ...
        elif args.model == 'ComplexRGCN':
            model = ComplexRGCN(embedding_size, config.hidden_dimension, out_dim, relation_num, config.dropout).to(device)
