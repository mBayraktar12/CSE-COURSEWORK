...
from torch_geometric.nn import RGCNConv,GCN2Conv,GCNConv,GATConv,SAGEConv,SGConv,RGATConv,GINConv, GlobalAttention
import torch.nn.functional as F


class ComplexRGCN(nn.Module):
    def __init__(self, embedding_dimension=16, hidden_dimension=128, out_dim=3, relation_num=2, dropout=0.3):
        super(ComplexRGCN, self).__init__()
        ...
        self.rgcn3 = RGCNConv(hidden_dimension, hidden_dimension, num_relations=relation_num)
    ...
    def forward(self, feature, edge_index, edge_type):
        x = self.linear_input(feature.to(torch.float32))
